...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # Since, dropout may add moise to prediction during test time.
        out = x
        #######################################################################
        #                            END OF YOUR CODE                         #
        #######################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    assert (H - HH) % stride == 0, 'Sanity check : Max Fool Failed in Height'
    assert (W - WW) % stride == 0, 'Sanity check: Max Pool Falied in Width'
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    running_var = bn_param.get('running _var', np.zeros((1, C, 1, 1), dtype = x.dtype))
    ...
    if mode == 'train':
        ...
        # Make record of means and variances in running parameters 
        running_mean = momentum * running_mean + (1 - momentum) * mu 
        running_var = momentum * running_var + (1 - momentum) * var 
